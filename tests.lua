...
local RunService = game:GetService("RunService")
local character = player.Character or player.CharacterAdded:Wait()
-- local getFarmSafe = require(game:GetService("ReplicatedStorage").Modules.GetFarmAsync)
-- local farm = getFarmSafe(game.Players.LocalPlayer)
-- getgenv().myFarm = farm.Name
...
-- -- ✅ Persistent config table (this is the magic)
-- getgenv().selectedSeedMap = getgenv().selectedSeedMap or {}
...
--     Searchable = true,
--     Text = "Select Seeds",
--     Default = getgenv().selectedSeedMap, -- Must be stable reference
--     Callback = function(selected)
--         -- 💡 Mutate the original map
--         for k in pairs(getgenv().selectedSeedMap) do
--             getgenv().selectedSeedMap[k] = nil
--         end
--         for k, v in pairs(selected) do
--             getgenv().selectedSeedMap[k] = v
--         end

--         -- Convert to array for use
--         getgenv().selectedSeeds = {}
--         for k, v in pairs(getgenv().selectedSeedMap) do
--             if v and k ~= "All" then
--                 table.insert(getgenv().selectedSeeds, k)
--             end
--         end

--         -- Handle select all
--         if getgenv().selectedSeedMap["All"] then
--             getgenv().selectedSeeds = {}
--             for _, seed in ipairs(seedList) do
--                 if seed ~= "All" then
--                     table.insert(getgenv().selectedSeeds, seed)
--                 end
--             end
--         end

--         Library:Notify("Selected: " .. table.concat(getgenv().selectedSeeds, ", "))
--     end
-- })
...
-- 	["Primal Egg"] = {
-- 		{ name = "Dinosaur Egg", itemType = "PetEgg", slot = 1},
-- 		{ name = "Bone Blossom", itemType = "Holdable", slot = 2}
-- 	}
-- }
...
creditsGroup:AddButton({
	...
	Tooltip = "You can reprot bugs or give suggestions here.",
    Func = function()
        setclipboard("https://discord.gg/WswepWXvr9")
...
-- 	-- print(crop)
--         if harvesting or currentTaskType and currentTaskType ~= "Harvest" then return end
-- 		currentTaskType = "Harvest"
--         harvesting = true
...
--         if planting or currentTaskType and currentTaskType ~= "Plant" then return end
...
--         if growing or currentTaskType and currentTaskType ~= "GrowPetToAge" then return end
-- 		currentTaskType = "GrowPetToAge"
--         growing = true
--         local PetsRE     = game:GetService("ReplicatedStorage").GameEvents.PetsService
...
-- 		if crafting or currentTaskType and currentTaskType ~= "Craft" then return end
-- 		crafting = true
-- 		currentTaskType = "Craft"
...
--         task.spawn(function()
--             while _G.autoDinoQuest and not Library.Unloaded do
-- 				containers = DataService:GetData().QuestContainers or {}
...
--         Tooltip = "Automate ALL dino quest for you",
--         Default = false,
--         Callback = function(v)
--             _G.autoDinoQuest = v
...
--                     end
-- 					-- if not found then
-- 					-- 	Library:Notify(wanted, "not found.")
-- 					-- end
--                 end
...
--auto feed
local feedBox = petTab:AddRightGroupbox("Feed")
-- local Players = game:GetService("Players")
-- local ReplicatedStorage = game:GetService("ReplicatedStorage")
-- local FeedRemote = game.ReplicatedStorage.GameEvents.ActivePetService
-- local player = Players.LocalPlayer
getgenv().PetModule = require(ReplicatedStorage.Modules.PetServices.ActivePetsService)

-- 🍎 Global selected fruit(s)
getgenv().selectedPlantsss = {}

-- 🌿 Dropdown UI
feedBox:AddDropdown("fruit_dropdown", {
	Text = "Fruit List",
	Values = allPlant,
	Tooltip = "Pick a fruit to use for feeding",
	Multi = true,
	Searchable = true,
	Callback = function(values)
		getgenv().selectedPlantsss = util.keysOf(values)
	end
})

getgenv().petHungerList = {}
getgenv().autoFeed = false
feedBox:AddToggle("auto_feed_toggle", {
	Text = "Auto Feed Hungry Pets",
	Default = false,
	Callback = function(state)
		getgenv().autoFeed = state
		if getgenv().autoFeed then
			Library:Notify("🍎 Auto Feed On!")
			util.feed()
		else
			Library:Notify("🛑 Auto Feed Off.")
		end
	end
})
...
-- -- 🔁 Example usage:
-- for uuid, hunger in pairs(petHungerList) do
-- 	if isHungry(uuid, 0) then
-- 		print("⚠️ Pet is hungry! UUID:", uuid, "| Hunger:", hunger)
-- 	end
-- end
